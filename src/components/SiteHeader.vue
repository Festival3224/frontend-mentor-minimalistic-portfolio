<template>
    ...
    <header class="bg-white text-white font-bold">    
        <nav class="flex justify-between items-center pl-4 pr-0 md:px-4 mx-auto max-w-screen-xl py-8">
            <!-- Logo  -->
            ...
            <transition name="mobil-outer"
                ...
                <ul class="py-8 text-white items-center gap-8 font-normal uppercase tracking-wider"
                    ...
                    <router-link @click.native="closeMobileNav" :to="{ name: 'Portfolio' }">portfolio</router-link>
                    <router-link @click.native="closeMobileNav" :to="{ name: 'Contact' }">contact me</router-link> 
                </ul>
            </transition>
        </nav>
    </header>
